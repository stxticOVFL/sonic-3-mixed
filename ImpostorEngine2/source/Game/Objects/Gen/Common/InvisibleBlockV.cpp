...
// Object Name: InvisibleBlockV.cpp

#include "InvisibleBlockV.h"

typedef IMath Math;

void InvisibleBlockV::Create() {
    Object::Create();
    Active = true;
    Priority = false;
...
void InvisibleBlockV::UpdateSubType() {
    Size = 1 + (SubType & 0xF0) / 0x10;
    W = (SubType & 0xF0) + 0x10;
    H = ((SubType & 0x0F) << 4) + 0x10;
    if (Deadly) {
...
uint8_t InvisibleBlockV::GetSubTypeIncrement() {
    return 1;
}

uint8_t InvisibleBlockV::GetSubTypeMax() {
    return 99;
}

void InvisibleBlockV::Render(int CamX, int CamY) {
    if (!Scene->maxLayer) {
...
int InvisibleBlockV::OnCollisionWithPlayer(int PlayerID, int HitFrom, int Data) {
    if (Painful) {
