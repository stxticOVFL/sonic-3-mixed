// Object ID: 0x28
// Object Name: InvisibleBlockH.cpp

#include "InvisibleBlockH.h"

typedef IMath Math;

void InvisibleBlockH::Create() {
    Object::Create();
    Active = true;
    Priority = false;
...
void InvisibleBlockH::UpdateSubType() {
    Size = 1 + (SubType & 0xF0) / 0x10;
    W = (SubType & 0xF0) + 0x10;
    H = ((SubType & 0x0F) << 4) + 0x10;
    if (Deadly) {
...
uint8_t InvisibleBlockH::GetSubTypeIncrement() {
    return 1;
}

uint8_t InvisibleBlockH::GetSubTypeMax() {
    return 99;
}

void InvisibleBlockH::Render(int CamX, int CamY) {
    if (!Scene->maxLayer) {
...
int InvisibleBlockH::OnCollisionWithPlayer(int PlayerID, int HitFrom, int Data) {
    if (Painful) {
