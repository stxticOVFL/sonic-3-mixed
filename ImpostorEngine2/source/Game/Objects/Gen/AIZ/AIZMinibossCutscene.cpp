// Object ID: 0x90
// Object Name: AIZMinibossCutscene.cpp

#include "AIZMinibossCutscene.h"

typedef IMath Math;

void AIZMinibossCutscene::Create() {
    Enemy::Create();
    Active = true;
    Priority = true;
    W = 48;
    ...
    Shot2 = -1;
    Shot3 = -1;
    Cutscene = ID == Obj_AIZMinibossCutscene;
    int i = 0;
...
void AIZMinibossCutscene::DoExplosionDebris() {
...
void AIZMinibossCutscene::HandleDamage() {
    if (HitCount > 0) {
        if (InvulnTimer > 0) {
            InvulnTimer -= 1;
            Invincible = true;
...
void AIZMinibossCutscene::Update() {
    switch (State) {
        case 2:
        if (Cutscene) {
...
void AIZMinibossCutscene::Render(int CamX, int CamY) {
    if (Cutscene) {
