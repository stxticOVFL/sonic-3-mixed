...
PUBLIC Scene_ExtrasMenu::Scene_ExtrasMenu(IApp* app, IGraphics* g) {
	App = app;
	G = g;

	Discord_UpdatePresence("Main Menu", "Extras Menu", "icon", false);

	selected = 1;
}

PUBLIC void Scene_ExtrasMenu::Init() {
	int at = 0;
...
PUBLIC void Scene_ExtrasMenu::Update() {
...
PUBLIC void Scene_ExtrasMenu::Render() {
	...
	G->DrawTextShadow(180, 112, "Extras Menu", 0xFFFFFF);
}
