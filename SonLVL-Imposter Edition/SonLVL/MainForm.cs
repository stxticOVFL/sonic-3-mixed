...
namespace SonicRetro.SonLVL.GUI
{
	public partial class MainForm : Form
	{
		...
		private void LevelToolStripMenuItem_Clicked(object sender, EventArgs e)
		{
			...
			LevelName = LevelData.Game.GetLevelInfo((string)((ToolStripMenuItem)sender).Tag).DisplayName;
		...
		public ushort SetPropertySelected(int bit, bool state, ushort tile)
		{
			if (state)
			{ tile |= (ushort)(1 << bit); }
			else
			{ tile &= (ushort)(~(1 << bit)); }
			Console.WriteLine("Tile = " + Convert.ToString(tile, 2));
			return tile;
		}

		private ushort FlipTile(FlipDirection direction, ushort tile)
		{
			//Console.WriteLine("TileBefore = " + Convert.ToString(tile, 2));
			tile ^= (ushort)direction;
			return tile;
			//Console.WriteLine("TileAfter = " + Convert.ToString(tile, 2));
		}

		public int GetSelectedTile(int TileIndex)
		{
			for (int i = 0; i < 6; ++i)
			{ TileIndex |= 1 << (10 + i); }
			return TileIndex;
		...
		public int SetBit(int pos, bool Set, int Value)
		{
			if (Set)
			{
				Value |= 1 << pos;
		...
		public Bitmap ExportToGif(string filepath, bool RS = false, bool SaveBitmap = true)
		{
			if (!loaded) { Console.WriteLine("Not Loaded!"); return null; }
			if (!highToolStripMenuItem.Checked && !lowToolStripMenuItem.Checked && !path1ToolStripMenuItem.Checked && !path2ToolStripMenuItem.Checked)
			{
				MessageBox.Show(this, "Cannot export blocks with nothing visible.", Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return null;
			...
			if (RS)
			{
				for (int i = 0; i < 64; i++)
				{
					pal.Entries[i] = LevelData.PaletteToColor(c / 16, c % 16, transparentBackgroundToolStripMenuItem.Checked);
					c++;
				}
				pal.Entries.Fill(Color.Magenta, 64, 128);
				pal.Entries.Fill(Color.Magenta, 192, 64);
			}
			else
			{
				for (int i = 128; i < 192; i++)
				...
				pal.Entries.Fill(Color.Magenta, 0, 128);
				pal.Entries.Fill(Color.Magenta, 192, 64);
			}

			BitmapBits bits = null;
			bits = new BitmapBits(16, 16384);
			for (int i = 0; i < LevelData.Blocks.Count; i++)
			{
				//if (highToolStripMenuItem.Checked & lowToolStripMenuItem.Checked)
				//{
				BitmapBits b = new BitmapBits(LevelData.CompBlockBmpBits[i]);
				b.IncrementIndexes(128);
				/*int pixel = 0;
				for (int h = 0; h < b.Height; h++)
				{
					for (int w = 0; w < b.Width; w++)
					{
						b.SafeSetPixel((byte)(b.GetPixelIndex(w,h) + 128), w, h);
						pixel++;
					}
				}*/
				bits.DrawBitmap(b, 0, i * 16);
				/*}
				else if (lowToolStripMenuItem.Checked)
				{
					BitmapBits b = new BitmapBits(LevelData.BlockBmpBits[i][0]);
					bits.DrawBitmap(b, 0, i * 16);
				}
				else if (highToolStripMenuItem.Checked)
				{
					BitmapBits b = new BitmapBits(LevelData.BlockBmpBits[i][1]);
					bits.DrawBitmap(b, 0, i * 16);
				}
				else
				{
					BitmapBits b = new BitmapBits(16,16);
					bits.DrawBitmap(b,0,i*16);
				}*/

			}
			if (SaveBitmap) bits.ToBitmap(pal).Save(filepath, ImageFormat.Gif);
			return bits.ToBitmap(pal);
		}

		public byte SwapNybble(byte nybble)
		{
			switch(nybble)
			{
				case 0:
		...
		private void exportTilesToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (dlg.ShowDialog(this) == DialogResult.OK)
			{
				ExportToGif(dlg.FileName);
			}
		}

		private void rSDKv5ToolStripMenuItem_Click(object sender, EventArgs e)
		{
			SaveFileDialog dlg = new SaveFileDialog();
			dlg.Title = "Select Folder to Place the stage in!";
			dlg.Filter = "Retro Engine v5 Scenes|Scene*.bin";

			RSDKv5.Scene SceneData = new RSDKv5.Scene();
			RSDKv5.TileConfig Tileconfig = new RSDKv5.TileConfig();
			RSDKv5.Stamps Stamps = new RSDKv5.Stamps();
			Bitmap Tiles;

			SceneData = new RSDKv5.Scene(Application.ExecutablePath.Replace(Path.GetFileName(Application.ExecutablePath), "") + "\\SceneBASE.bin");

			if (dlg.ShowDialog(this) == DialogResult.OK)
			{
				SceneData.EditorMetadata.BinName = "ManiacStamps.bin";

				byte[,] FGlayout = null;
				FGlayout = LevelData.Layout.FGLayout;

				byte[,] BGlayout = null;
				BGlayout = LevelData.Layout.BGLayout;
				...
				for (int i = 0; i < SceneData.Layers[layer].Height; i++)
				{
				...
				{
					if (c >= 256) break;
					RSDKv5.Stamps.TileChunk Stamp = new RSDKv5.Stamps.TileChunk(8);
					for (int y = 0; y < 8; y++)
					{
						for (int x = 0; x < 8; x++)
						{
							ChunkBlock blk = LevelData.Chunks[c].Blocks[x, y];

							ushort Tile = (ushort)GetSelectedTile(blk.Block);

							byte ColInd1 = LevelData.GetColInd1(blk.Block);
							byte ColInd2 = LevelData.GetColInd2(blk.Block);

							bool FlipX = false;
							bool FlipY = false;

							if (blk.XFlip) { FlipX = true; }

							if (blk.YFlip) { FlipY = true; }

							bool SolidCollisionATop = false;
							if (blk.Solid1 == Solidity.TopSolid) { SolidCollisionATop = true; }
							bool SolidCollisionAOther = false;
							if (blk.Solid1 == Solidity.LRBSolid) { SolidCollisionAOther = true; }
							if (blk.Solid1 == Solidity.AllSolid) { SolidCollisionAOther = true; SolidCollisionATop = true; }

							bool SolidCollisionBTop = false;
							if (blk.Solid1 == Solidity.TopSolid) { SolidCollisionBTop = true; }
							bool SolidCollisionBOther = false;
							if (blk.Solid1 == Solidity.LRBSolid) { SolidCollisionBOther = true; }
							...
							Tile = (ushort)SetBit(12, SolidCollisionATop, Tile);
							Tile = (ushort)SetBit(13, SolidCollisionAOther, Tile);
							Tile = (ushort)SetBit(14, SolidCollisionBTop, Tile);
							Tile = (ushort)SetBit(15, SolidCollisionBOther, Tile);

							if (x < Stamp.ChunkSize && y < Stamp.ChunkSize)
							{
								if (!HighP)
								{
									Stamp.TileMapA[y][x] = Tile;
									Stamp.TileMapB[y][x] = 0xFFFF;
								}
								else
								{
									Stamp.TileMapA[y][x] = 0xFFFF;
									Stamp.TileMapB[y][x] = Tile;
								}
							}
						}
					}
				...
				//Where the (Background) black magic happens!
				for (int h = 0; h < LevelData.BGHeight; h++)
				{
					for (int w = 0; w < LevelData.BGWidth; w++)
					{
						for (int y = 0; y < 8; y++)
						{
							for (int x = 0; x < 8; x++)
							{
								...
								tx = (w * 8) + x;
								ty = (h * 8) + y;
								ChunkBlock blk = LevelData.Chunks[BGlayout[w, h]].Blocks[x, y];
								ushort Tile = (ushort)GetSelectedTile(blk.Block);
								...
								if (blk.Block > 0)
								{
									if (tx < SceneData.Layers[0].Width && ty < SceneData.Layers[0].Height)
									{ SceneData.Layers[0].Tiles[ty][tx] = Tile; }
								}
								else
								{
									if (tx < SceneData.Layers[0].Width && ty < SceneData.Layers[0].Height)
									{ SceneData.Layers[0].Tiles[ty][tx] = 0xffff; }
								}
							}
						}
					}
				...
				SceneData.Layers[FGLow].Resize((ushort)(LevelData.FGWidth * 8), (ushort)(LevelData.FGHeight * 8));

				SceneData.Layers[FGLow].Tiles = new ushort[SceneData.Layers[FGLow].Height][];
				for (int i = 0; i < SceneData.Layers[FGLow].Height; i++)
				{
					SceneData.Layers[FGLow].Tiles[i] = new ushort[SceneData.Layers[FGLow].Width];
				}

				SceneData.Layers[FGHigh].Resize((ushort)(LevelData.FGWidth * 8), (ushort)(LevelData.FGHeight * 8));

				SceneData.Layers[FGHigh].Tiles = new ushort[SceneData.Layers[FGHigh].Height][];
				for (int i = 0; i < SceneData.Layers[FGHigh].Height; i++)
				{
				...
				{
					for (int w = 0; w < LevelData.FGWidth; w++)
					{
						for (int y = 0; y < 8; y++)
						{
							for (int x = 0; x < 8; x++)
							{
								...
								tx = (w * 8) + x;
								ty = (h * 8) + y;
								ChunkBlock blk = LevelData.Chunks[FGlayout[w, h]].Blocks[x, y];
								...
								if (!HighP && blk.Block > 0)
								{
									if (tx < SceneData.Layers[FGLow].Width && ty < SceneData.Layers[FGLow].Height)
									{ SceneData.Layers[FGLow].Tiles[ty][tx] = Tile; }
								}
								else if (HighP && blk.Block > 0)
								{
									if (tx < SceneData.Layers[FGHigh].Width && ty < SceneData.Layers[FGHigh].Height)
									{ SceneData.Layers[FGHigh].Tiles[ty][tx] = Tile; }
								}
								else if (blk.Block == 0)
								{
									if (tx < SceneData.Layers[FGLow].Width && ty < SceneData.Layers[FGLow].Height && tx < SceneData.Layers[FGHigh].Width && ty < SceneData.Layers[FGHigh].Height)
									{
										SceneData.Layers[FGLow].Tiles[ty][tx] = 0xffff;
										SceneData.Layers[FGHigh].Tiles[ty][tx] = 0xffff;
									}
								}
							}
						}
				...
				for (int i = 0; i < ObjectNames.Count; i++)
				{
					...
					if (obj.Name.Name != "PathSwapper")
					{
						...
						attributeInfof.Type = RSDKv5.AttributeTypes.UINT8;

						RSDKv5.AttributeInfo attributeInfo = new RSDKv5.AttributeInfo();
					...
					{
						RSDKv5.AttributeInfo attributeInfo1 = new RSDKv5.AttributeInfo();
						attributeInfo1.Name = new RSDKv5.NameIdentifier("Angle");
						attributeInfo1.Type = RSDKv5.AttributeTypes.UINT32;

						RSDKv5.AttributeInfo attributeInfo2 = new RSDKv5.AttributeInfo();
						attributeInfo2.Name = new RSDKv5.NameIdentifier("Flags");
						attributeInfo2.Type = RSDKv5.AttributeTypes.UINT32;

						RSDKv5.AttributeInfo attributeInfo3 = new RSDKv5.AttributeInfo();
						attributeInfo3.Name = new RSDKv5.NameIdentifier("OnPath");
						attributeInfo3.Type = RSDKv5.AttributeTypes.BOOL;

						RSDKv5.AttributeInfo attributeInfo4 = new RSDKv5.AttributeInfo();
						attributeInfo4.Name = new RSDKv5.NameIdentifier("Size");
						attributeInfo4.Type = RSDKv5.AttributeTypes.UINT32;

						obj.Attributes.Add(attributeInfo1);
						obj.Attributes.Add(attributeInfo2);
						obj.Attributes.Add(attributeInfo3);
						obj.Attributes.Add(attributeInfo4);
					}
				...
				for (int i = 0; i < LevelData.Objects.Count; i++)
				{
					...
					if (ent.Object.Name.Name != "PathSwapper")
					{
				...
				for (int i = 0; i < LevelData.Rings.Count; i++)
				{
					RSDKv5.SceneEntity ent = new RSDKv5.SceneEntity(Ringobj, SlotID++);
					ent.Position.X.High = (short)LevelData.Rings[i].X;
					ent.Position.Y.High = (short)LevelData.Rings[i].Y;
				...
				for (int i = 0; i < LevelData.StartPositions.Count; i++)
				{
					RSDKv5.SceneEntity ent = new RSDKv5.SceneEntity(Playerobj,SlotID++);
					ent.Position.X.High = (short)LevelData.StartPositions[i].X;
				...
				SceneData.Objects.Add(Blankobj);
				SceneData.Objects.Add(Ringobj);
				...
				stageConfig.ObjectsNames.Add("Ring");

				for (int i = 0; i < ObjectNames.Count; i++)
				{
					stageConfig.ObjectsNames.Add(ObjectNames[i]);
				}

				for (int i = 0; i < LevelData.Blocks.Count; i++)
				{
					Tileconfig.CollisionPath1[i].slopeAngle = LevelData.Angles[LevelData.ColInds1[i]];

					for (int c = 0; c < 16; c++)
					{
						Tileconfig.CollisionPath1[i].HasCollision[c] = true;
						sbyte cd = LevelData.ColArr1[LevelData.ColInds1[i]][c];
						if (cd < 0)
						{
							Tileconfig.CollisionPath1[i].IsCeiling = true;
							cd = (sbyte)(-cd);
						}
						if (cd == 0)
						{
							Tileconfig.CollisionPath1[i].HasCollision[c] = false;
						}
						else
						{
					...
					for (int c = 0; c < 16; c++)
					{
						Tileconfig.CollisionPath2[i].HasCollision[c] = true;
						sbyte cd = LevelData.ColArr1[LevelData.ColInds2[i]][c];
						if (cd < 0)
						{
							Tileconfig.CollisionPath2[i].IsCeiling = true;
							cd = (sbyte)(-cd);
						}
						if (cd == 0)
						{
							Tileconfig.CollisionPath2[i].HasCollision[c] = false;
						}
						else
						{
				...
				string Dirname = dlg.FileName.Replace(Path.GetFileName(dlg.FileName), "");
				SceneData.Write(dlg.FileName);
				RSDKv5.Writer scWriter = new RSDKv5.Writer(Dirname + "//Stageconfig.bin");
				stageConfig.Write(scWriter);
				scWriter.Close();
				Tiles = ExportToGif(Dirname + "//16x16Tiles.gif");

				Bitmap Tiles512 = new Bitmap(512, 512);
				using (Graphics g = Graphics.FromImage(Tiles512))
				{
					int index = 0;
					for (int y = 0; y < 32; y++)
					{
						for (int x = 0; x < 32; x++)
						{
							g.DrawImage(CropImage(Tiles, new Rectangle(0, index * 16, 16, 16)), new Point(x * 16, y * 16));
							index++;
						}
					}
				}
				Tiles512.Save(Dirname + "//16x16Tiles512.png");

				Tileconfig.Write(Dirname + "//Tileconfig.bin");
				Stamps.Write(Dirname + "//ManiacStamps.bin");

				StreamWriter NameWriter = new StreamWriter(File.OpenWrite(Dirname + "//ObjectNames.txt"));
				for (int i = 0; i < ObjectNames.Count; i++) 
				{
					NameWriter.WriteLine(ObjectNames[i]);
				}
				NameWriter.Close();
			}
		}
